...
export default function addBraveRoutes(r: Partial<SettingsRoutes>) {
  ...
  if (!r.BASIC) {
    console.error('[Asil Settings Overrides] Routes: could not find BASIC page')
  }
  if (pageVisibility.getStarted) {
    ...
    // We re-section people page into getStarted section (see people_page Asil
  ...
  if (r.ADVANCED) {
    ...
    console.error('[Asil Settings Overrides] Could not find ADVANCED page', r)
  }
  ...
  if (r.SITE_SETTINGS) {
    ...
    if (r.SITE_SETTINGS_ADS) {
      ...
      console.error('[Asil Settings Overrides] could not find expected route site_settings_ads')
    }
  } else if (!isGuest) {
    console.error('[Asil Settings Overrides] Routes: could not find SITE_SETTINGS page')
  }
  ...
  if (r.AUTOFILL && r.ADVANCED) {
    ...
    console.error('[Asil Settings Overrides] Could not move autofill route to advanced route', r)
  }
  // Safety check route is moved to advanced.
  if (r.SAFETY_CHECK && r.ADVANCED) {
    r.SAFETY_CHECK.parent = r.ADVANCED
  } else if (!isGuest) {
    console.error('[Asil Settings Overrides] Could not move safety check route to advanced route', r)
  }
}
