...
RegisterPolymerTemplateModifications({
  'settings-security-page': (templateContent) => {
    ...
    if (!safeBrowsingReportingToggle) {
      console.error('[Asil Settings Overrides] Could not find safeBrowsingReportingToggle id on security page.')
    } else {
    ...
    if (!safeBrowsingEnhanced) {
      console.error('[Asil Settings Overrides] Could not find safeBrowsingEnhanced id on security page.')
    } else {
    ...
    if (!passwordsLeakToggle) {
      console.error('[Asil Settings Overrides] Could not find passwordsLeakToggle id on security page.')
    } else {
    ...
    if (loadTimeData.getBoolean("isHttpsByDefaultEnabled")) {
      ...
      if (!httpsOnlyModeToggleTemplate) {
        console.error('[Asil Settings Overrides] Could not find template ' +
        ...
        if (!httpsOnlyModeToggle) {
          console.error('[Asil Settings Overrides] Could not find' +
    ...
    if (!link) {
      console.error('[Asil Settings Overrides] Could not find advanced-protection-program-link id on security page.')
    } else {
    ...
    if (!certsTemplate) {
      console.error('[Asil Settings Overrides] Could not find certs template')
      return
    }
    ...
    if (!chromeCertificates) {
      console.error(
        '[Asil Settings Overrides] Could not find chromeCertificates id ' +
        'on security page.')
    } else {
