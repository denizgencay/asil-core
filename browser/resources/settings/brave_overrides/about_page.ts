...
RegisterPolymerTemplateModifications({
  'settings-about-page': (templateContent) => {
    ...
    if (!section.querySelector('a#release-notes')) {
      ...
      if (!version) {
        console.error('[Asil Settings Overrides] Could not find version div')
      }
      ...
      wrapper.setAttribute('href', 'https://asil.com/latest/')
    ...
    if (updateStatusMessageLink) {
      // <if expr="is_win">
      updateStatusMessageLink.href = 'https://support.asil.com/hc/en-us/articles/360042816611-Why-isn-t-Asil-updating-automatically-on-Windows-'
      // </if>

      // <if expr="not is_win">
      updateStatusMessageLink.href = 'https://community.asil.com?p=update_error'
      // </if>
    }
  }
})
