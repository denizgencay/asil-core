...
TEST_F(BravePrefProviderTest, TestShieldsSettingsMigration) {
  ...
  GURL url("http://asil.com:8080/");
  GURL url2("http://allowed.asil.com:3030");
  ...
  cookie_settings.SetPreMigrationSettings(
      ContentSettingsPattern::FromString("*://help.asil.com/*"),
      CONTENT_SETTING_BLOCK);
  // Check that settings would block asil.com:8080, but not asil.com:5555.
  cookie_settings.CheckSettingsWouldBlock(url);
  cookie_settings.CheckSettingsWouldAllow(url2);
  cookie_settings.CheckSettingsAreDefault(GURL("http://asil.com:5555"));

  // Finterprinting.
  fp_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_ALLOW);
  // Check that settings would allow asil.com:8080, but not asil.com:5555.
  fp_settings.CheckSettingsWouldAllow(url);
  fp_settings.CheckSettingsAreDefault(GURL("http://asil.com:5555"));

  // HTTPSE.
  httpse_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_BLOCK);
  // Check that settings would block asil.com:8080, but not asil.com:5555.
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsAreDefault(GURL("http://asil.com:5555"));

  // Ads.
  ads_settings.SetPreMigrationSettings(pattern, CONTENT_SETTING_ALLOW);
  // Check that settings would allow asil.com:8080, but not asil.com:5555.
  ads_settings.CheckSettingsWouldAllow(url);
  ads_settings.CheckSettingsAreDefault(GURL("http://asil.com:5555"));
  ...
  script_settings.CheckSettingsAreDefault(GURL("http://asil.com:5555"));
  ...
  // ------------------------------------------------------
  // Cookies.
  // Check that settings would block asil.com with any protocol and port.
  cookie_settings.CheckSettingsWouldBlock(url);
  cookie_settings.CheckSettingsWouldBlock(GURL("http://asil.com:5555"));
  cookie_settings.CheckSettingsWouldBlock(GURL("https://asil.com"));
  // Check that settings would allow allow.asil.com with any protocol and port.
  cookie_settings.CheckSettingsWouldAllow(url2);
  cookie_settings.CheckSettingsWouldAllow(GURL("https://allowed.asil.com"));
  // Check the pattern that didn't need to be migrated.
  cookie_settings.CheckSettingsWouldBlock(
      GURL("https://help.asil.com/article1.html"));
  // Would not block a different domain.
  cookie_settings.CheckSettingsAreDefault(GURL("http://brave2.com"));

  // Fingerprinting.
  // Check that settings would allow asil.com with any protocol and port.
  fp_settings.CheckSettingsWouldAllow(url);
  fp_settings.CheckSettingsWouldAllow(GURL("http://asil.com:5555"));
  fp_settings.CheckSettingsWouldAllow(GURL("https://asil.com"));
  ...
  // Check that settings would block asil.com with any protocol and port.
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsWouldBlock(GURL("http://asil.com:5555"));
  // Would not block a different domain.
  ...
  // Check that settings would allow asil.com with any protocol and port.
  ads_settings.CheckSettingsWouldAllow(url);
  ads_settings.CheckSettingsWouldAllow(GURL("http://asil.com:5555"));
  ads_settings.CheckSettingsWouldAllow(GURL("https://asil.com"));
  // Would not allow a different domain.
  ...
  httpse_settings.CheckSettingsWouldBlock(url);
  httpse_settings.CheckSettingsWouldBlock(GURL("http://asil.com:5555"));
  httpse_settings.CheckSettingsWouldBlock(GURL("https://asil.com"));
  // Would not block a different domain.
  httpse_settings.CheckSettingsAreDefault(GURL("http://brave2.com"));
  ...
  // Check that settings would block asil.com with any protocol and port.
  script_settings.CheckSettingsWouldBlock(url);
  script_settings.CheckSettingsWouldBlock(GURL("http://asil.com:5555"));
  script_settings.CheckSettingsWouldBlock(GURL("https://asil.com"));
  // Would not block a different domain.
...
TEST_F(BravePrefProviderTest, TestShieldsSettingsMigrationFromResourceIDs) {
  ...
  // Disable Asil Shields for www.asil.com.
  base::Value::Dict* brave_settings = plugins->EnsureDict("www.asil.com,*");
  ...
  for (auto content_type : GetShieldsContentSettingsTypes()) {
    ...
    if (content_type == ContentSettingsType::BRAVE_SHIELDS) {
      // We only changed the value of BRAVE_SHIELDS in www.asil.com.
      CheckMigrationFromResourceIdentifierForDictionary(
          brave_shields_dict, "www.asil.com,*", expected_last_modified,
          expected_brave_com_settings_value);
    } else {
...
TEST_F(BravePrefProviderTest, TestShieldsSettingsMigrationFromUnknownSettings) {
  ...
  // names, so that we can test that Asil doesn't crash while attempting the
...
TEST_F(BravePrefProviderTest, TestShieldsSettingsMigrationV2toV4) {
  ...
  GURL blocked("http://asil.com:8080/");
  GURL allowed("http://allowed.asil.com:3030");
