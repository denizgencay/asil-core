...
TEST(BraveNewsDirectFeed, CantAddDuplicateFeed) {
  TestingPrefServiceSimple prefs;
  BraveNewsController::RegisterProfilePrefs(prefs.registry());
...
TEST(BraveNewsDirectFeed, CanExplicitlySetId) {
  TestingPrefServiceSimple prefs;
  BraveNewsController::RegisterProfilePrefs(prefs.registry());
...
TEST(BraveNewsDirectFeed, EmptyTitleFallsBackToFeedSource) {
  TestingPrefServiceSimple prefs;
  BraveNewsController::RegisterProfilePrefs(prefs.registry());
  ...
  ASSERT_EQ(1u, dict.size());
  for (const auto&& [key, value] : dict) {
    auto* title = value.FindStringKey(prefs::kBraveTodayDirectFeedsKeyTitle);
    EXPECT_EQ(kFeedSource, *title);
  }
}

TEST(BraveNewsDirectFeed, DirectFeedCanBeInspectedAndRemoved) {
  TestingPrefServiceSimple prefs;
  BraveNewsController::RegisterProfilePrefs(prefs.registry());
